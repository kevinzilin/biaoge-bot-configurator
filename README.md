...
所有平台脚本统一为“薄壳脚本 + Python 核心”。日常使用优先运行项目根目录下的安装/启动脚本，排查问题或部署常驻运行时再直接调用 `scripts/` 下的脚本。
...
```bash
# Windows
.venv\Scripts\python.exe scripts\launch.py --interactive --preflight-only

# macOS / Linux
.venv/bin/python scripts/launch.py --interactive --preflight-only
```
...
# macOS
bash scripts/disable_autostart_macos.sh

# Linux
bash scripts/disable_autostart_linux.sh
```

Windows 可追加 `-StopRunning` 尝试结束当前计划任务；Linux 可追加 `--stop` 停止当前 systemd 服务，系统级服务对应追加 `--system`。
