...
def _is_path_under(path: str, base_dir: str) -> bool:
    ...
    if node_id or error_type == "prompt_outputs_failed_validation":
        ...
        if "invalid image file" in str(node_details or "").lower():
            hint = (
                ...
                "如果你要传本机文件，可以写 `image=@./pics/a.jpg` 或使用当前系统的绝对路径。"
            )
        elif node_details:
...
def get_help_text() -> str:
    return (
        ...
        "/wf <workflow> images=@./pics/a.jpg  —— 本机文件上传后再执行（支持用引号包住带空格的路径）\n"
    ...
    for it in items:
        ...
        if provider == "runninghub":
            ...
            if fv:
                out.append(fv)
            try:
                if os.path.exists(saved) and _is_path_under(saved, ctx.settings.temp_download_dir):
                    os.remove(saved)
            except Exception:
                pass
    ...
    *,
    ...
    if os.name == "nt":
        if p.startswith("/") or p.startswith("\\"):
            raise RuntimeError(f"file not found: {p} (looks like a POSIX path; this bot is running on {_platform_name()}, please use a local path for this machine, or remove '@' if the file already exists on the ComfyUI machine)")
    else:
        if re.match(r"^[A-Za-z]:\\\\", p):
            raise RuntimeError(f"file not found: {p} (looks like a Windows path; this bot is running on {_platform_name()}, please use a local path for this machine, or remove '@' if the file already exists on the ComfyUI machine)")
...
def _read_split_task_limit_cfg(raw_cfg: Any) -> tuple[int, str]:
    cfg = raw_cfg if isinstance(raw_cfg, dict) else {}
    limit_raw = cfg.get("splitTaskLimit") or cfg.get("split_task_limit") or 50
    try:
        limit = int(str(limit_raw).strip())
    except Exception:
        limit = 50
    limit = max(1, min(10000, limit))
    strategy_raw = cfg.get("splitTaskLimitStrategy") or cfg.get("split_task_limit_strategy") or "truncate"
    strategy = str(strategy_raw or "truncate").strip().lower()
    if strategy not in ("truncate", "error"):
        strategy = "truncate"
    return limit, strategy


def _apply_split_task_limit(
    *,
    items: list[Any],
    limit: int,
    strategy: str,
    ...
    if strategy == "error":
        detail_text = ""
        if detail:
            detail_text = f"（明细={detail}）"
        raise RuntimeError(
            f"{reason}后预计生成 {total} 个子任务，超过 splitTaskLimit={limit}{detail_text}。"
            "请提高 splitTaskLimit，或调小各关联表的 maxItems。"
        )
    logging.warning(
        "split tasks truncated: workflow=%s record_id=%s reason=%s total_generated=%s limit=%s kept=%s detail=%s",
        workflow_key,
        record_id,
        reason,
        total,
...
async def run_workflow(
    ...
    *,
    ...
    split_task_limit, split_task_limit_strategy = _read_split_task_limit_cfg(raw_cfg)
    ...
    if use_record_fields:
        try:
            ...
            if split_groups:
                ...
                group_sizes = [len(items) for _, items in split_groups]
                logging.info(
                    "split tasks generated by cartesian product: workflow=%s record_id=%s groups=%s total=%s limit=%s strategy=%s",
                    workflow_key,
                    record_id,
                    group_sizes,
                    len(cartesian_items),
                    split_task_limit,
                    split_task_limit_strategy,
                )
                # 取最后一个作为 split_param（保持向后兼容的行为）
                split_param = split_groups[-1][0] if split_groups else None
                relation_split_items, _, _ = _apply_split_task_limit(
                    items=cartesian_items,
                    limit=split_task_limit,
                    strategy=split_task_limit_strategy,
    ...
    split_items: list[dict[str, Any]] = list(relation_split_items) if relation_split_items else []
    if not split_items and split_param and split_param in merged and split_param not in params:
        v0 = merged.get(split_param)
        if isinstance(v0, list):
            raw_split_values = [str(x).strip() for x in v0 if str(x).strip()]
            if len(raw_split_values) > 1:
                logging.info(
                    "split tasks generated by list param: workflow=%s record_id=%s param=%s total=%s limit=%s strategy=%s",
                    workflow_key,
                    record_id,
                    split_param,
                    len(raw_split_values),
                    split_task_limit,
                    split_task_limit_strategy,
                )
            split_values, _, _ = _apply_split_task_limit(
                items=raw_split_values,
                limit=split_task_limit,
                strategy=split_task_limit_strategy,
                workflow_key=workflow_key,
                record_id=record_id,
                reason=f"参数 `{split_param}` 列表拆分",
                detail=split_param,
            )
    ...
    try:
        ...
        for idx, sp in enumerate(runs):
            ...
            if provider == "runninghub":
                ...
                if ctx.settings.task_request_dump_dir:
                    try:
                        save_dir = ctx.settings.task_request_dump_dir
    ...
    *,
    ...
    if use_record_fields:
        ...
        if split_groups:
            ...
            # 取最后一个作为 split_param（保持向后兼容的行为）
            split_param = split_groups[-1][0] if split_groups else None
            relation_split_items, split_generated_total, split_truncated = _apply_split_task_limit(
                items=cartesian_items,
                limit=split_task_limit,
                strategy=split_task_limit_strategy,
                workflow_key=workflow_key,
    ...
    if not split_items and split_param and split_param in merged and split_param not in params:
        ...
        if isinstance(v0, list):
            raw_split_values = [str(x).strip() for x in v0 if str(x).strip()]
            split_values, split_generated_total, split_truncated = _apply_split_task_limit(
                items=raw_split_values,
                limit=split_task_limit,
                strategy=split_task_limit_strategy,
                workflow_key=workflow_key,
    ...
    elif split_items and split_generated_total <= 0:
        split_generated_total = len(split_items)
    ...
    return {
        ...
        "provider": provider,
        "comfyui_base_url": comfyui_base_url,
        "splitTaskLimit": split_task_limit,
        "splitTaskLimitStrategy": split_task_limit_strategy,
        "generatedRunCount": split_generated_total or len(out_runs),
        "plannedRunCount": len(out_runs),
        "splitTruncated": split_truncated or ((split_generated_total or len(out_runs)) > len(out_runs)),
        "runs": out_runs,
    }
